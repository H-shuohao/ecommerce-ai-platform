...
- 自动化测试：AI Core `123/123` 通过，告警 Webhook `2/2` 通过。
...
当前重点完成了 AI Core、售前 Agent、带持久化异步任务的内容 Agent、轻量数据中台、评测、MCP、支持真实文件接入的素材中心增强基础版、直播切片规划基础版和容器化闭环。FFmpeg物理裁剪、云对象存储、生成式图片/视频和模型微调仍属于后续路线，不在当前简历中描述为已完成能力。
