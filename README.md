...
| 项目4 直播切片 Agent | 可运行版 | 带时间戳转写、高光规划、边界校验、FFprobe时长校验、FFmpeg物理裁剪与素材登记 |
...
- **直播切片 Agent（可运行版）**：根据带时间戳转写识别直播高光，校验时间范围；对素材中心的本地源视频使用FFprobe校验真实时长，通过FFmpeg完成H.264/AAC物理裁剪，并把生成的MP4重新登记到素材中心等待人工审核。
...
- 自动化测试：AI Core `127/127` 通过，告警 Webhook `2/2` 通过。
...
当前重点完成了 AI Core、售前 Agent、带持久化异步任务的内容 Agent、轻量数据中台、评测、MCP、支持真实文件接入的素材中心、包含FFmpeg物理裁剪的直播切片可运行版和容器化闭环。ASR自动转写编排、云对象存储、生成式图片/视频和模型微调仍属于后续路线，不在当前简历中描述为已完成能力。
...
4. 在现有上传、切片规划和FFmpeg裁剪基础上，补齐ASR自动转写、异步任务编排和云对象存储。
