...
| 项目4 直播切片 Agent | 可运行版 | 带时间戳转写、高光规划、持久化异步任务、断点续跑、FFprobe时长校验、FFmpeg物理裁剪与素材登记 |
| 项目5 内容运营 Agent | 核心功能完成 | 多平台文案、事实约束、异步任务、幂等提交、失败重试、草稿、合规检查、人工审核 |
| 项目6 MCP共享能力层 | 核心功能完成 | Streamable HTTP、5 Tools、1 Resource、1 Prompt |
| 项目7 多模态素材中心 | 增强基础版 | image/video/text元数据、真实文件上传、类型/大小校验、SHA-256去重、本地持久化与下载 |
| 项目8 评测与可观测 | 评测版完成 | Run轨迹、固定评测集、工具准确率、HTTP指标、并发测试与基线比较 |
...
- **真实流式对话**：Agent 执行状态、工具调用和模型 Token 通过 NDJSON 持续返回，Web Demo 边生成边展示；明确商品库存问题使用确定性路由，降低选错工具和额外模型调用。
...
- **持久化异步内容任务**：耗时的模型生成请求可先返回 `202 Accepted`，任务状态写入 SQLite；支持 `X-Idempotency-Key` 防止重复提交、失败任务有限重试，以及服务重启后的中断任务恢复。
- **多模态素材中心（增强基础版）**：统一登记图片、视频和文本素材元数据，支持真实文件上传、商品/类型/标签检索、文件签名与大小校验、SHA-256内容寻址去重、本地持久化和受控下载。
- **直播切片 Agent（可运行版）**：根据带时间戳转写识别直播高光；通过持久化异步任务编排LLM选段和多个FFmpeg裁剪，支持202接单、幂等冲突检测、逐片保存进度、失败续跑和服务重启恢复；生成的MP4重新登记到素材中心等待人工审核。
...
- 自动化测试：AI Core `132/132` 通过，告警 Webhook `2/2` 通过。
...
每个 HTTP 响应都包含 `X-Request-ID`。客户端也可以主动传入合法的 `X-Request-ID`，便于把前端报错与后端 JSON 日志关联起来。

错误响应在保留原有 `detail` 字段的同时，增加统一结构：
...
```text
POST /api/v1/agents/presales/chat/stream
```

接口使用 `application/x-ndjson` 按行返回事件，典型顺序如下：

```text
session → status → tool → delta... → done
```

- `session`：返回由后端创建或复用的会话编号；
- `status`：表示 Agent 正在规划和执行；
- `tool`：返回本轮实际调用的工具和参数；
- `delta`：大模型生成的单段文本，前端收到后立即追加；
- `done`：返回完整回答、工具轨迹、运行编号和总耗时；
...
当前重点完成了 AI Core、售前 Agent、带持久化异步任务的内容 Agent、轻量数据中台、评测、MCP、支持真实文件接入的素材中心、包含持久化异步编排与FFmpeg物理裁剪的直播切片可运行版和容器化闭环。ASR自动转写编排、云对象存储、生成式图片/视频和模型微调仍属于后续路线，不在当前简历中描述为已完成能力。
...
4. 在现有上传、持久化异步切片流水线和FFmpeg裁剪基础上，补齐批量ASR自动转写和云对象存储。
